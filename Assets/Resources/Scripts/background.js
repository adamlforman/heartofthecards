/*function init() {
	var modelObject = GameObject.CreatePrimitive(PrimitiveType.Plane);	// Create a quad object for holding the tile texture.
	modelObject.SetActive(false);										// Turn off the object so its script doesn't do anything until we're ready.
	
	model = modelObject.AddComponent("backgroundModel");					// Add a terrainModel script to control visuals of the terrain.
