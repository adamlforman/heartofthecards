...
function init(o : GameObject) {
	...
	if(owner.name == "Explosion"){
		...
		GetComponent.<Renderer>().material.color = Color(1,0,0);												// Set the color (easy way to tint things).
	}
		...
		textureName = "Textures/Comet";
		GetComponent.<Renderer>().material.color = Color(0.8,0.1,0.1);
	}
...
function Update () {
	...
	if (owner.name == "Enemy Comet") {
		...
		GetComponent.<Renderer>().material.color = Color(1-getColor,0,0);
	}
	else if (owner.name == "Comet") {
		getColor = owner.GetComponent(PlayerSpell).delayTimer;
		GetComponent.<Renderer>().material.color = Color(0,0,1-getColor);
	}
	else if (owner.name == "Enemy Lava") {
		getColor = owner.GetComponent(Temporary).life;
		GetComponent.<Renderer>().material.color = Color(getColor/20,0,0);
	}	
}
